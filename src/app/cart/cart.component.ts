...
export class CartComponent implements OnInit {
  ...
  packCb(form: NgForm) {
    ...
    // Get the logged-in user ID from local storage and convert it to a number
    var stringToConvert = localStorage.getItem('loggeduser');
    var numberValue = Number(stringToConvert);
    console.log(numberValue);

    // Set additional values for the form submission
    ...
    // Call the service method to add the package
    this.userSer.addpackages(form.value).subscribe(
      (data: string) => {
        this.msg = data;
        form.reset();
        // Navigate to the payment page after successfully adding the package
        // Navigate to the payment page after successfully adding the package
